...
function get_selected_or_cursor_word()
  ...
  local finalText = (selection:gsub("^%s*(.-)%s*$", "%1")) == "" and vim.fn.expand("<cword>") or
      selection:gsub("^%s*(.-)%s*$", "%1")
  print([==[finaltext]==], vim.inspect(finalText)) -- __AUTO_GENERATED_PRINT_VAR_END__
  return finalText
end

-- @ cannot test by reruning while in v mode (will get n mode)
local function runGetVisual()
  -- will not update next run
  -- local text = require("utils.input").get_selected_or_cursor_word() --  mode n  cannot test since run with command ?
  local text = get_selected_or_cursor_word()           --  mode n  cannot test since run with command ?
