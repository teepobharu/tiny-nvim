...
if is_noplugin then
  ...
  vim.keymap.set(
    "n",
    "<leader>vc",
    ':!nvim --clean -c "set rtp+=$XDG_CONFIG_HOME/$NVIM_APPNAME" -c "lua require(\\"config.mykeymaps\\")"<CR>',
    { noremap = true, silent = true, desc = "Start Neovim clean with mykeymaps" }
  )
  ...
  vim.api.nvim_create_user_command("ReloadLuaInit", function(opts)
    reloadLuaInit(opts.bang)
  end, { bang = true, desc = "Reload Lua init.lua or open it with !" })
  vim.keymap.set("n", "Q", ":q!<CR>", { noremap = true, silent = false, desc = "Force Quit" })
  vim.keymap.set(
    "n",
    "<leader>rl",
    ":ReloadLuaInit<CR>",
    { noremap = true, silent = true, desc = "Reload Lua init.lua" }
  )
  vim.keymap.set(
    "n",
    "<leader>rL",
